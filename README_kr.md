...
사내 PII 패턴은 `PiiDetector` 빈을 등록하여 확장합니다. 등록한 빈은 `List<PiiDetector>` 자동 주입으로 수집되며, 자신의 `id()` 가 활성 `ComplianceProfile.piiProviders()` (또는 `audit.compliance.pii.providers`) 에 포함되는 순간 활성화됩니다.
...
전체 SPI 명세는 [다른 국가로 확장 — SPI](#다른-국가로-확장--spi) 섹션 (ADR-007) 참고.
